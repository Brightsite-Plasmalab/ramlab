...
class AbInitioMolecule(HitranCompatibleMolecule):
    ...
    def transitions_metadata(
        ...
        transitions.initial_degeneracy = self.degeneracy(state_initial)
        transitions.final_degeneracy = self.degeneracy(state_final)

        transitions.initial_E = self.E(state_initial)
        transitions.final_E = self.E(state_final)
        ...
        transitions.molecule_number = int(self.molecule_number)
        transitions.isotope_number = self.isotope_number
        return transitions
    ...
    def transitions(
        ...
        for state in [state_initial, state_final]:
            state.degeneracy = self.degeneracy(state)
            state.E = self.E(state)

        transitions = Transitions.for_states(state_initial, state_final)

        transitions = self.transitions_metadata(
            transitions, laser_wavelength, polarisation
        )
        return self.transitions_calculate(laser_wavelength, transitions, polarisation)
