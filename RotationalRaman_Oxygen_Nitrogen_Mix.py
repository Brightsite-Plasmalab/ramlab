...
directory = "C:\\Users\\P70085588\\Data\\Raman_Spectoscopy\\2025\\3\\31_N2\\"
...
directory = "C:\\Users\\P70085588\\Data\\Raman_Spectoscopy\\2025\\3\\12\\15mm_700W_20SLM\\"
directory_params = "C:\\Users\\P70085588\\Data\\Raman_Spectoscopy\\Par2meters\\Rotational_Raman\\"

name_sig = "534nm_22s_60um_1700lmm_2mm"
name_bg = "534nm_22s_60um_1700lmm_2mm_bg"
...
fitParameters.add("A", value=1, min=0.5, max=1.2, vary=True)
fitParameters.add("T", value=2900, min=292, max=5000, vary=True)
fitParameters.add("y0", value=0.005, min=0.0, max=0.006, vary=True)
#fitParameters.add("w_g_r", value=1, min=0, max=2, vary=True)
#fitParameters.add("w_l_r", value=1, min=0, max=2, vary=True)

fitParameters.add('x_N2', value=0, min=0,max=1, vary=True)
fitParameters.add('x_O2', value=1, min=0,max=1, vary=False)
fitParameters.add('x_O', value=0.0, min=0,max=1, vary=True)

fitParameters.add('k0', value=10.5, min=4, max=14, vary=True)
fitParameters.add('k1', value=1.0144, min=1.005, max=1.02, vary=True)
fitParameters.add('k2', value=0.3493, min=0.27, max=0.5, vary=True)
...
def spectral_fit_residuals(pars: Parameters, meas, meas_full, test1=False, test2=False):
    ...
    T_sim = pars['T']
    y0 = pars['y0']
    w_g = np.sqrt(params[0]**2 + calc_new_w(T_sim,wav_len_av, "Combined")**2)
    w_l = params[1]
    k0 = pars['k0']
    k1 = pars['k1']
    k2 = pars['k2']
    ...
    #I_tot_comb = A / np.nanmax(I_tot_comb) * I_tot_comb + y0
...
if out.params['T'].value >= 2300:
    ...
    x_N2 = out.params['x_N2'].value / (out.params['x_N2'].value + out.params['x_O2'].value)
    x_O2 = out.params['x_O2'].value / (out.params['x_N2'].value + out.params['x_O2'].value)
    x_O = 0

print("Gas Temperature:\t\t", str(round(out.params['T'].value,0)))
print("FWHM of Gaussian:\t\t",str(round(np.sqrt(params[0]**2 + calc_new_w(out.params['T'].value,wav_len_av, "Combined")**2),4)))
print("FWHM of Lorentzian:\t\t", str(round(params[1],4)))
